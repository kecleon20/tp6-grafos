...
class BinaryTree:
    ...
    def inorden_tipos_pokemon(self,tipo):
        def __inorden_tipos_pokemon(root):
            if root is not None:
                __inorden_tipos_pokemon(root.left)
                if root.value == tipo:
                    name = root.other_values['name']
                    print(name)
                __inorden_tipos_pokemon(root.right)

        __inorden_tipos_pokemon(self.root)
    
    # punto f 
    def contar_electrico_pokemon(self):
        def __contar_electrico_pokemon(root):
            count = 0
            if root is not None:
                if root.value == 'electrico':
                    count = 1
                count += __contar_electrico_pokemon(root.left)
                count += __contar_electrico_pokemon(root.right)
            return count

        return __contar_electrico_pokemon(self.root)
   
    def contar_acero_pokemon(self):
        def __contar_acero_pokemon(root):
            count = 0
            if root is not None:
                if root.value == 'acero':
                    count = 1
                count += __contar_acero_pokemon(root.left)
                count += __contar_acero_pokemon(root.right)
            return count

        return __contar_acero_pokemon(self.root)
